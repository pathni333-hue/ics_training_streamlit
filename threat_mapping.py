...
def app():
    ...
    if st.button('Submit answer'):
        ...
        pick_set = set(picks)
        if pick_set == correct_set or pick_set.issuperset(correct_set):
            st.session_state['score'] += 1
            st.success('Correct! Full mitigations selected.')
        else:
            st.error('Not fully correct. Correct mitigations are:')
            st.write(cur['mitigations'])

    st.write(f"Score: {st.session_state['score']} / {st.session_state['attempts']}")
    # Heatmap of coverage (simple)
    coverage = []
    for t in TECHNIQUES:
        covered = False
        # naive: if any mitigation selected previously matches technique mitigations, mark covered
